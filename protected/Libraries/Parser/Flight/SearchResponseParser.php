...
class SearchResponseParser extends BaseResponseParser implements ResponseParser
{
        ...
        foreach ($items as $item) {
            ...
            if ($i == 0 && $isFirstFlight) {
                ...
                */
                ...
                $object->departureTime = $exploded[8];
                ...
                // set transit price 0
                /*if(in_array($apiController->request->airline, ['TPJT'])){
                        if($schedule->isTransit){
                                $object->price = intval(0);
                        }
                }*/
    ...
    public static function getDetailRute($airport, $detail, $departureDate)
    {
        ...
        foreach (explode('>', trim($detail)) as $value) {
            ...
            //https://static.scash.bz/Travel/asset/maskapai/TP
            //https://static.scash.bz/Travel/assets/images/flighticons/
            $s[] = (object) array(
                "flightIcon" => "https://static.scash.bz/Travel/asset/maskapai/TP" . substr($code[0], 0, 2) . ".png",
                ...
                for ($i = sizeof($value->classes); $i < sizeof($value->detailTitle); $i++) {
                    $arrayClass[] = array(
                        (object) array(
                            ...
                            'class' => '',
                            'price' => 0,
                            'departureTime' => $value->detailTitle[$i]->depart,
