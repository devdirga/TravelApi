...
class HttpConnect
{
    public static function send($message, $timeout = 210)
    {
        ...
        if (strpos($message, "*FA22044*") !== false || strpos($message, "*FT1013*") !== false) {
            curl_setopt($curl, CURLOPT_URL, '10.0.0.20/FMSSWeb/mpin1');
            curl_setopt($curl, CURLOPT_PORT, '21080');
        } else if (preg_match('/\FA32670*\b/', $message)) {
            curl_setopt($curl, CURLOPT_URL, '10.0.0.20/FMSSWeb/mpin1');
            curl_setopt($curl, CURLOPT_PORT, '21080');
        } else if (preg_match('/\HH158972*\b/', $message)) { //khusus sqiva
    ...
    public static function sendToCore($method, $server, $port, $path, $message, $timeout = 240)
    { //methode TRUE = POST
