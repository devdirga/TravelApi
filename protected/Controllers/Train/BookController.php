...
use Travel\Libraries\Parser\Train\BookResponseParser;
...
class BookController extends APIController
{
    ...
    public function indexAction()
    {
        ...
        foreach ($this->request->passengers->adults as $adult) {
            ...
            if (trim($phone) == "") //jika adult number kosong, ambil dari yg pertama
                $message->set(TrainMessage::FIELD_ADULT_MOBILE1 + ($i * 4), $first_phone . $i);
            else
                $message->set(TrainMessage::FIELD_ADULT_MOBILE1 + ($i * 4), $phone);
            $i++;
        }
